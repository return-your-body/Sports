	...
	<script>
		...
		function drawCharts(data) {
			...
			if (data.work?.length) {
				...
				$('#workChart').replaceWith('<div class="text-danger">⚠ 無資料</div>');
			}
			...
			if (data.leave?.length && data.leave_types?.length) {
				...
				$('#leaveChart').replaceWith('<div class="text-danger">⚠ 無資料</div>');
			}

			renderPie('itemChart', itemCounts, ['item_name', 'doctor_name', 'count'], '項目數比例詳細', 'itemTableContainer');
			renderPie('appointmentChart', appointmentCounts, ['doctor_name', 'count'], '預約人數比例詳細', 'appointmentTableContainer');
			renderPie('incomeChart', incomeData, ['item_name', 'doctor_name', 'income'], '收入統計詳細', 'incomeTableContainer');
		}

		function renderPie(canvasId, data, fields, title, containerId) {
			const ctx = document.getElementById(canvasId)?.getContext('2d');
			if (!ctx || !data.length) {
				$(`#${containerId}`).html('<div class="text-danger">⚠ 無資料</div>');
				return;
			}
			...
			const chartMap = { 'itemChart': 'itemChart', 'appointmentChart': 'appointmentChart', 'incomeChart': 'incomeChart' };
			if (window[chartMap[canvasId]]) window[chartMap[canvasId]].destroy();
			window[chartMap[canvasId]] = new Chart(ctx, {
				type: 'pie',
				data: {
					labels: labels,
