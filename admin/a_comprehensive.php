...
.chart-row { display: flex; flex-wrap: wrap; gap: 16px; justify-content: space-between; }
...
<!-- JavaScript 請改為呼叫 drawCharts() 和 drawPieCharts() 等 -->
<script>
  ...
  // 圓餅圖共用繪製函式
  function renderPie(canvasId, title, dataset, labelKey, valueKey, tableId) {
    const ctx = document.getElementById(canvasId).getContext('2d');

    if (!dataset || dataset.length === 0) {
      document.getElementById(tableId).innerHTML = `<p class="text-danger text-center">⚠ 無資料</p>`;
      new Chart(ctx, {
        type: 'pie',
      ...
      options: {
        onClick: function (evt, item) {
          if (item.length) {
            ...
            renderPieTable(label, value, tableId, title);
          }
        }
      }
    });

    document.getElementById(tableId).innerHTML = `<p class="text-muted text-center">請點選圖表查看詳細資料</p>`;
  }
