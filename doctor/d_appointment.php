						...
						if (calendarData[fullDate]) {
							calendarData[fullDate].forEach(shift => {
								...
								if (adjustedShift) {
									...
									timeInfo.textContent = `${adjustedShift.go_time} - ${adjustedShift.off_time}`;

									const button = document.createElement('button');
									button.textContent = '查看';
									button.onclick = () => openModal(fullDate, adjustedShift.go_time, adjustedShift.off_time);
									timeInfo.appendChild(button);

									cell.appendChild(timeInfo);
								} else {
									const noSchedule = document.createElement('div');
									noSchedule.textContent = '請假';
