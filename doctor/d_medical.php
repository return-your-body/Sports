...
<ㄎ?php session_start(); if (!isset($_SESSION["登入狀態"])) { header("Location: ../index.html"); exit; } // 防止頁面被瀏覽器緩存
    header("Cache-Control: no-store, no-cache, must-revalidate, max-age=0"); header("Expires: Sat, 26 Jul 1997 05:00:00
    GMT"); header("Pragma: no-cache"); // 檢查 "帳號" 是否存在於 $_SESSION 中 if (isset($_SESSION["帳號"])) { // 獲取用戶帳號
    $帳號=$_SESSION['帳號']; // 資料庫連接 require '../db.php' ; // 接收搜尋參數 $search_name=isset($_GET['search_name']) ?
    mysqli_real_escape_string($link, trim($_GET['search_name'])) : '' ; // 查詢該帳號的詳細資料 $sql="SELECT user.account, doctor.doctor AS name 
            FROM user 
            JOIN doctor ON user.user_id = doctor.user_id 
            WHERE user.account = ?" ; $stmt=mysqli_prepare($link, $sql); mysqli_stmt_bind_param($stmt, "s" , $帳號);
    mysqli_stmt_execute($stmt); $result=mysqli_stmt_get_result($stmt); if (mysqli_num_rows($result)> 0) {
    // 抓取對應姓名
    ...
    if (empty($item_ids)) {
    echo "
    <script>alert('請至少選擇一項診療項目！');</script>";
    } else {
    $success = false;
    ...
    if ($count == 0) {
    echo "
    <script>alert('診療項目 (ID: $item_id) 不存在！');</script>";
    exit;
    }
    ...
    echo "
    <script>alert('診療紀錄新增失敗：" . mysqli_error($link) . "');</script>";
    exit;
    }
    $success = true;
    }
    ...
    echo "
    <script>alert('更新預約狀態失敗：" . mysqli_error($link) . "');</script>";
    exit;
    }
    }

    echo "
    <script>alert('診療紀錄新增成功！'); window.location.href = 'd_appointment-records.php';</script>";
    }
    }
    ?>
    ...
    <body>
        ...
        <div class="page">
            <header class="section page-header">
                ...
                <div class="rd-navbar-wrap rd-navbar-centered">
                    <nav class="rd-navbar" data-layout="rd-navbar-fixed" data-sm-layout="rd-navbar-fixed"
                        ...
                        data-xxxl-layout="rd-navbar-static" data-lg-device-layout="rd-navbar-fixed"
                        data-xl-device-layout="rd-navbar-static" data-xxl-device-layout="rd-navbar-static"
                        data-xxxl-device-layout="rd-navbar-static" data-stick-up-offset="1px"
                        data-sm-stick-up-offset="1px" data-md-stick-up-offset="1px" data-lg-stick-up-offset="1px"
                        data-xl-stick-up-offset="1px" data-xxl-stick-up-offset="1px" data-xxx-lstick-up-offset="1px"
                        data-stick-up="true">
                            ...
                            <div class="rd-navbar-panel">
                                ...
                                <div class="rd-navbar-brand">
                                    <!--Brand--><a class="brand-name" href="d_index.php"><img class="logo-default"
                                            ...
                                            loading="lazy" /><img class="logo-inverse"
                                            src="images/logo-inverse-172x36.png" alt="" width="86" height="18"
                                            loading="lazy" /></a>
                                </div>
                            </div>
            ...
            <div class="form-container">
                ...
                <h2>新增診療紀錄</h2>
                <form action="d_medical.php?id=<?php echo htmlspecialchars($_GET['id']); ?>" method="POST">
